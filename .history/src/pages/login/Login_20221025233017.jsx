...
export default function Login({ authService, onLogin }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    if (signup) {
      const { loginIdError, duplicateError, passwordVerificationError } =
        signForm;
      if (
        loginIdError ||
        duplicateError ||
      ...
      authService.headerTokenLogin(signForm);
    }
  };
  ...
  useEffect(() => {
    ...
    const time = setTimeout(() => {
      if (signup) {
        authService.idDuplicateVerification(signForm.loginId).then((data) => {
          return data.result
            ? setVerificationError((e) => ({ ...e, duplicateError: false }))
