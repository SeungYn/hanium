...
export default function ChatPage({ chatService, title, groupService }) {
  ...
  const [owner, setOwner] = useState(() =>
    state?.data.owner === user.nickname ? true : false
  );
  ...
  const [kickConfirmModal, setKickConfirmModel] = useState(false);
  //현재 클릭된 멤버
  ...
  //그룹 정보 갱신하기
  const HandlerGroupUpdate = () => {
    groupService.getGroupInfo(groupInfo.clubId).then((data) => {
      console.log(data);
      //setGroupInfo({...data}
    });
  };

  const HandlerMemberKick = () => {
    groupService
      .kickMember(groupInfo.clubId, clickedMember.memberId)
      .then((data) => setGroupInfo((g) => ({ ...data })))
      .catch(setError);
  };
    ...
    <main className={styles.container}>
      {kickConfirmModal && (
        <Confirm
          ...
          message={`정말로 ${clickedMember.nickname}님을 강퇴하시겠습니까?`}
        />
      )}
      <ChatSideBar
        ...
        owner={owner}
        HandlerMemberClick={(m) => {
          setClickedMember({ ...m });
          setKickConfirmModel(true);
        }}
        myInfo={user}
      />
      <ChatHeader
      ...
      <button
        onClick={() => {
          setGroupInfo({
            ...
            owner: '2',
            memberInfoDtos: [
              { nickname: '2', sex: 'MALE', dept: '컴퓨터 공학과', sno: 17 },
            ],
          });
        }}
      >
