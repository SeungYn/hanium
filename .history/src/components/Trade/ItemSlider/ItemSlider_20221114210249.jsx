...
export default function ItemSlider({ imgs }) {
  ...
  const HandlePrevBtn = () => {
    const { width } = widthRef.current.getBoundingClientRect();
    console.log(sliderRef.style.left);
    if (currentIdx >= 0) {
      sliderRef.style.left = -currentIdx * width + 'px';
      sliderRef.style.transition = `${0.5}s ease-out`;
    }
    if (currentIdx === 0) {
      setTimeout(function () {
        sliderRef.style.left = -(imgs.length - 1) * width + 'px';
        sliderRef.style.transition = `${0}s ease-out`;
      }, 500);
